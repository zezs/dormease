...
router.get('/home/booking', isLoggedIn, async (req, res) => {
    ...
    res.render('users/createbooking.ejs', { user });
})
//Bookings of washing machine
router.post("/createbooking", isLoggedIn, async (req, res) => {
    const { date, time } = req.body;
    console.log(req.body)
    const user = req.user;
    console.log(user);
    const bookings = await Bookings.find({ username: req.user.username })
    for (let i = 0; i < bookings.length; i++) {
        if (bookings[i].username === user.username) {
